...
class Paths:
    ...
    CLIP_LENGTHS_FILE = os.getenv(
        "TASK_INFO_FILE", "round2/metadata/clip_lengths_mouse_triplets.json"
    )
    FRAME_NUMBER_MAP = os.getenv(
        "FRAME_MAP_FILE", "round2/metadata/mouse_frame_number_map.npy"
    )
...
class AIcrowdEvaluator:
    ...
    def merge_ant_scores(self, results_df):
        ...
        merge_df = results_df[
            results_df["Task ID"].apply(lambda tid: tid in tasks_to_merge)
        ]
        results_df = results_df[
            results_df["Task ID"].apply(lambda tid: tid not in tasks_to_merge)
        ]
        priv_score = merge_df["Private Score"].mean()
        pub_score = merge_df["Public Score"].mean()
        new_df = pd.DataFrame(
            {
                "Task ID": ["interactor_type"],
                "Private Score": [priv_score],
                "Public Score": [pub_score],
                "Metric": ["f1_score"],
            }
        )
    ...
    def _evaluate(self, client_payload, _context={}):  # pylint: disable=W0102
        ...
        if self.task_name == "mouse":
            ...
            Paths.FRAME_NUMBER_MAP = os.getenv(
                "FRAME_MAP_FILE", "round2/metadata/mouse_round2_frame_number_map.npy"
            )
            ...
            Paths.FRAME_NUMBER_MAP = os.getenv(
                "FRAME_MAP_FILE", "round2/metadata/antbeetle_frame_number_map.npy"
            )
            embedding_max_size = 128
            test_size = 0.1
            ...
            Paths.FRAME_NUMBER_MAP = (
                "./round2/metadata/mouse_new_splits/frame_number_map.npy"
            )
            embedding_max_size = 128
            test_size = 0.1
        ...
        train_and_eval.run_all_tasks(
            Paths, test_size, client_payload["subsample_factor"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="mouse",
        choices=["mouse", "antbeetle", "mouse_behaviors"],
    )
    ...
    results.to_csv(
        os.path.join(
            args.output_dir, task + "-round-2-" + "-" + sub_file.split("/")[-1] + ".csv"
        ),
        index=False,
    )
