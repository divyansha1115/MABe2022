...
def predict_single_task_multiseed(
    data_splitter, task_id, model_paths, Constants, split_keys
):
    start = time.time()
...
def eval_single_task_multiseed(
    data_splitter, task_id, model_paths, Constants, task_is_sequence_level
):
    ...
    res = predict_single_task_multiseed(
        data_splitter, task_id, model_paths, Constants, split_keys=["publicTest"]
    )
    ...
    res = predict_single_task_multiseed(
        data_splitter, task_id, model_paths, Constants, split_keys=["privateTest"]
    )
...
def run_all_tasks(Constants, test_size, subsample_factor):
    ...
    start = time.time()
    data_splitter = MABeDataSplitter(
...
if __name__ == "__main__":
    ...
    class Constants:
        ...
        SPLIT_INFO_FILE = os.getenv(
            "SPLIT_INFO_FILE", "./example_data/example_split.json"
        )
