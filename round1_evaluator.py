...
class Paths:
    ...
    CLIP_LENGTHS_FILE = os.getenv(
        "CLIP_LENGTHS_FILE", "round1/metadata/clip_lengths_mouse_triplets.json"
    )
    FRAME_NUMBER_MAP = os.getenv(
        "FRAME_MAP_FILE", "round1/metadata/mouse_frame_number_map.npy"
    )
...
def validate_submission(
    submission_file_path, clip_lengths_file, embedding_max_size, frame_map_file
):
    ...
    for key, clip_length in clip_lengths.items():
        ...
        if not end - start == clip_length:
            raise ValueError(
                f"Frame number map for clip {key} doesn't match clip length"
            )

    if not len(submission["embeddings"]) == total_clip_length:
        raise ValueError(
            f"Emebddings length doesn't match submission clips total length"
        )
...
class AIcrowdEvaluator:
    ...
    def average_singlefly_tasks(self, results_df):
        ...
        contains_singletask = results_df["Task ID"].apply(
            lambda name: "singletask" in name
        )

        allfly_df = results_df[~contains_singletask]
        singlefly_df = results_df[contains_singletask]
        sf_task_keys = np.unique(
            singlefly_df["Task ID"].apply(lambda name: name.split("_singletask")[0])
        )

        singlefly_avg = []
        other_column_keys = list(singlefly_df.columns)[4:]
        for tk in sf_task_keys:
            task_df = singlefly_df[
                singlefly_df["Task ID"].apply(lambda name: tk in name)
            ]
            ...
            singlefly_avg.append(
                (tk, private_score, public_score, "f1_score", *others_vals)
            )
    ...
    def _evaluate(self, client_payload, _context={}):
        ...
        if self.task_name == "mouse":
            ...
            Paths.CLIP_LENGTHS_FILE = os.getenv(
                "CLIP_LENGTHS_FILE", "round1/metadata/clip_lengths_fruit_flies.json"
            )
            Paths.FRAME_NUMBER_MAP = os.getenv(
                "FRAME_MAP_FILE", "round1/metadata/flies_frame_number_map.npy"
            )
...
if __name__ == "__main__":
    ...
    results.to_csv(
        os.path.join(
            args.output_dir, task + "-round-1-" + "-" + sub_file.split("/")[-1] + ".csv"
        ),
        index=False,
    )
